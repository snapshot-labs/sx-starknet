...
describe('Felt Utils:', () => {
  let testFeltUtils: StarknetContract;
  ...
  before(async function () {
    ...
    testFeltUtils = await testFeltUtilsFactory.deploy();
  });
  ...
  it('The library should covert 4 64 bit words to a Uint256', async () => {
    ...
    const { uint256: uint256 } = await testFeltUtils.call('testWordsToUint256', {
      word1: word1,
      word2: word2,
      word3: word3,
      word4: word4,
  ...
  it('The library should pack 4 32 bit numbers into a felt', async () => {
    ...
    const { packed: packed } = await testFeltUtils.call('testPackFelt', {
      num1: num1,
      num2: num2,
      num3: num3,
      num4: num4,
    ...
    const {
      ...
      num3: _num3,
      num4: _num4,
    } = await testFeltUtils.call('testUnpackFelt', { packed: packed });
