...
async function baseSetup() {
    ...
    const GnosisSafeL2 = await hre.ethers.getContractFactory(
      ...
      const masterSnapshotXModule = await SnapshotXContract.deploy(
        ...
        const initData = masterSnapshotXModule.interface.encodeFunctionData(
          "setUp",
          [encodedInitParams]
        );    
     
      const masterCopyAddress = masterSnapshotXModule.address
        .toLowerCase()
        .replace(/^0x/, "");
...
async function receiveProposalTest(SnapshotXModule: any, safe: any) {
    ...
    const execution_details = ethers.utils.keccak256(abiCoder.encode(["bytes32[]"], [[tx_hash1, tx_hash2]]));
    const has_passed = 1;
  ...
  // can use the safe and a cancel proposal role
    describe("setUp", async () => {
        ...
        it("can initialize and set up the SnapshotX module", async () => {
            ...
            expect(await SnapshotXModule.starknetCore()).to.equal("0xB0aC056995C4904a9cc04A6Cc3a864A9E9A7d3a9");
    ...
    describe("Getters", async () => {
        it("The module should return the number of transactions in a proposal", async () => {
            const { SnapshotXModule, safe } = await baseSetup();
            await receiveProposalTest(SnapshotXModule, safe);
        ...
        it("The module should return whether a transaction in a proposal has been executed", async () => {
            ...
            await SnapshotXModule.executeProposalTx(
                    0, 
                    tx1.to, 
                    tx1.value, 
                    tx1.data, 
    ...
    describe("Transaction Hashes", async () => {
        it("should hash transactions correctly", async () => {
            ...
            expect(
                await SnapshotXModule.getTransactionHash(
                    tx1.to,
                    tx1.value,
                    tx1.data,
                    tx1.operation
    ...
    describe("Proposal Receival", async () => {
        it("The module can receive a proposal", async () => {
            ...
            const {tx_hash1, tx_hash2} = await receiveProposalTest(SnapshotXModule, safe);
    ...
    describe("Proposal Execution", async () => {
        it("The module can execute one transaction in a proposal", async () => {
            const { SnapshotXModule, safe } = await baseSetup();
            const {tx_hash1} = await receiveProposalTest(SnapshotXModule, safe);
            
            await expect(
                SnapshotXModule.executeProposalTx(
                    0, 
                    tx1.to, 
                    tx1.value, 
                    tx1.data, 
            ...
            ).to.emit(SnapshotXModule, "TransactionExecuted").withArgs(0, tx_hash1);
        ...
        it("The module can execute all transactions in a proposal individually", async () => {
            ...
            ).to.emit(SnapshotXModule, "TransactionExecuted").withArgs(0, tx_hash2)
             .to.emit(SnapshotXModule, "ProposalExecuted").withArgs(0);
