...
describe('Starknet Signature Authenticator Tests', function () {
  ...
  before(async function () {
    ...
    const { sierraCode: sig_sierra, casmCode: sig_casm } =
      await getCompiledCode('sx_StarkSigAuthenticator');
    const sig_calldata = CallData.compile({
      name: domain_name,
    ...
    starkSigAuthenticator = new Contract(
      sig_sierra.abi,
      sig_response.deploy.contract_address,
      provider
    );
    ...
    const { sierraCode: voting_sierra, casmCode: voting_casm } =
      await getCompiledCode('sx_VanillaVotingStrategy');
    const voting_response = await account.declareAndDeploy({
      contract: voting_sierra,
      casm: voting_casm
    });
    vanillaVotingStrategy = new Contract(
      voting_sierra.abi,
    ...
    const { sierraCode: proposal_sierra, casmCode: proposal_casm } =
      await getCompiledCode('sx_VanillaProposalValidationStrategy');
    const proposal_response = await account.declareAndDeploy({
      contract: proposal_sierra,
      casm: proposal_casm
    });
    vanillaProposalValidationStrategy = new Contract(
      proposal_sierra.abi,
    ...
    const { sierraCode: space_sierra, casmCode: space_casm } =
      await getCompiledCode('sx_Space');
    ...
    _voting_strategies = [
      { address: vanillaVotingStrategy.address, params: [] }
    ];
  ...
  it('can authenticate a proposal, a vote, and a proposal update', async () => {
    ...
    const proposeRes = await starkSigAuthenticator.authenticate_propose(
      proposeSignature,
      proposeMsg.space,
      proposeMsg.author,
      proposeMsg.metadataUri,
      proposeMsg.executionStrategy,
      proposeMsg.userProposalValidationParams,
      proposeMsg.salt
    );
    ...
    const updateRes = await starkSigAuthenticator.authenticate_update_proposal(
      updateSignature,
      updateProposalMsg.space,
      updateProposalMsg.author,
      updateProposalMsg.proposalId,
      updateProposalMsg.executionStrategy,
      updateProposalMsg.metadataUri,
      updateProposalMsg.salt
    );
    ...
    const voteRes = await starkSigAuthenticator.authenticate_vote(
      voteSignature,
      voteMsg.space,
      voteMsg.voter,
      voteMsg.proposalId,
      choice,
      voteMsg.userVotingStrategies,
      voteMsg.metadataUri
    );
  ...
  it('should revert if an incorrect signature is used', async () => {
    ...
    try {
      ...
      const invalidProposeSig = [
        invalidProposeSignature.r,
        invalidProposeSignature.s
      ];
      const invalidProposeRes =
        await starkSigAuthenticator.authenticate_propose(
          invalidProposeSig,
          proposeMsg.space,
          proposeMsg.author,
          proposeMsg.metadataUri,
    ...
    try {
      ...
      const invalidUpdateProposalSig = [
        invalidUpdateProposalSignature.r,
        invalidUpdateProposalSignature.s
      ];
      const invalidUpdateProposalRes =
        await starkSigAuthenticator.authenticate_update_proposal(
          invalidUpdateProposalSig,
          updateProposalMsg.space,
          updateProposalMsg.author,
          updateProposalMsg.proposalId,
          updateProposalMsg.executionStrategy,
      ...
      await provider.waitForTransaction(
        invalidUpdateProposalRes.transaction_hash
      );
      expect.fail('Should have failed');
    } catch (err: any) {
      expect((err as Error).message).to.contain(
    ...
    const updateProposalSig = [
      updateProposalSignature.r,
      updateProposalSignature.s
    ];
    const updateProposalRes =
      await starkSigAuthenticator.authenticate_update_proposal(
        updateProposalSig,
        updateProposalMsg.space,
        updateProposalMsg.author,
    ...
    try {
      ...
      const invalidVoteRes = await starkSigAuthenticator.authenticate_vote(
        invalidVoteSig,
        voteMsg.space,
        voteMsg.voter,
        voteMsg.proposalId,
