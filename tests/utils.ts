...
export async function safeWithL1AvatarExecutionStrategySetup(
  ...
  const masterL1AvatarExecutionStrategy =
    await L1AvatarExecutionStrategyFactory.deploy();

  const initData = masterL1AvatarExecutionStrategy.interface.encodeFunctionData(
    'setUp',
    [
      await safeSigner.getAddress(),
  ...
  const masterCopyAddress = (await masterL1AvatarExecutionStrategy.getAddress())
    .toLowerCase()
    .replace(/^0x/, '');
  ...
  expect(
    await moduleFactory.deployModule(
      await masterL1AvatarExecutionStrategy.getAddress(),
      initData,
      '0x01'
    )
  )
  ...
  const L1AvatarExecutionStrategy =
    L1AvatarExecutionStrategyFactory.attach(expectedAddress);
...
export async function increaseEthBlockchainTime(
  networkUrl: string,
  seconds: number
) {
  await axios({
